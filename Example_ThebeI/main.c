...
#include "../drivers/ThebeI/ThebeI.h"
#include "../drivers/WE-common.h"
#include "../drivers/global/global.h"


static void ThebeI_test_function();
static void ThebeI_rx_test();
...
void *main_Thread(void *pArgs)
{
   ...
   ThebeI_rx_test();
#else
    /* function to test all functions of the ThebeI driver */
    ThebeI_test_function();
#endif
...
static void ThebeI_test_function()
{
    ...
    if(ret)
    {
        fprintf (stdout, COLOR_CYAN "ThebeI driver version %d.%d.%d\n" COLOR_RESET,driver_version[0],driver_version[1],driver_version[2]);
    }
    delay(500);

    /* initialize the module ThebeI */
    ret = ThebeI_Init(9600, ThebeI_PIN_RESET, RXcallback, AddressMode_0);
    Debug_out("ThebeI init", ret);
    ...
    if(ret)
    {
        ...
        ret = ThebeI_GetSerialNumber(serial_number);
        ...
        ret = ThebeI_Get(ThebeI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, &data_length);
        ...
        data[0] = 15; /* 15dbm */
        data_length = 1;
        ret = ThebeI_Set(ThebeI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, data_length);
        Debug_out("Set TXpower to 15 dBm", ret);
        delay(500);
        ...
        ret = ThebeI_Get(ThebeI_USERSETTING_MEMPOSITION_NUMRETRYS, numretries, &numretries_len);
        ...
        ret = ThebeI_Get(ThebeI_USERSETTING_MEMPOSITION_OP_MODE, opMode, &opMode_len);
        ...
        ret = ThebeI_Get(ThebeI_USERSETTING_MEMPOSITION_CFG_FLAGS, cfgFlags, &cfgFlags_len);
        ...
        ret = ThebeI_Get(ThebeI_USERSETTING_MEMPOSITION_UARTBAUDRATE, baudrate, &baudrate_len);
        ...
        ret = ThebeI_SetVolatile_TXPower(10);
        Debug_out("Set volatile TXpower to 10 dBm", ret);
        delay(500);

        ret = ThebeI_SetVolatile_Channel(10);
        Debug_out("Set volatile Channel to 10", ret);
        delay(500);

        ret = ThebeI_SetVolatile_DestNetID(1);
        Debug_out("Set volatile DestNetID to 1", ret);
        delay(500);

        ret = ThebeI_SetVolatile_DestAddr(0x00,0x00);
        ...
        int8_t txpower = 0;
        ret = ThebeI_GetDefaultTXPower(&txpower);
        Debug_out("ThebeI_GetDefaultTXPower", ret);
        fprintf (stdout,"TXPower is %d dBm\n",txpower);
        delay(500);

        ret = ThebeI_SetDefaultTXPower(20);
        Debug_out("ThebeI_SetDefaultTXPower to 20dBm", ret);
        delay(500);
        ...
        uint8_t channel = 0;
        ret = ThebeI_GetDefaultRFChannel(&channel);
        Debug_out("ThebeI_GetDefaultRFChannel", ret);
        fprintf (stdout,"RFChannel is %d\n",channel);
        delay(500);

        ret = ThebeI_SetDefaultRFChannel(9);
        Debug_out("ThebeI_SetDefaultRFChannel to 9", ret);
        delay(500);
        ...
        uint8_t srcnetid = 0;
        ret = ThebeI_GetSourceNetID(&srcnetid);
        Debug_out("ThebeI_GetSourceNetID", ret);
        fprintf (stdout,"Source net ID is %d\n",srcnetid);
        delay(500);

        ret = ThebeI_SetSourceNetID(17);
        Debug_out("ThebeI_SetSourceNetID to 17", ret);
        delay(500);
        ...
        uint8_t destnetid = 0;
        ret = ThebeI_GetDefaultDestNetID(&destnetid);
        Debug_out("ThebeI_GetDefaultDestNetID", ret);
        fprintf (stdout,"Destination net ID is 0x%x\n",destnetid);
        delay(500);

        ret = ThebeI_SetDefaultDestNetID(0x00);
        Debug_out("ThebeI_SetDefaultDestNetID to 0x12", ret);
        delay(500);
        ...
        ret = ThebeI_GetSourceAddr(&srcaddr_lsb);
        Debug_out("ThebeI_GetSourceAddr", ret);
        fprintf (stdout,"Source address is 0x%x\n",srcaddr_lsb);
        delay(500);

        ret = ThebeI_SetSourceAddr(0x13,0x14);
        Debug_out("ThebeI_SetSourceAddr to 0x13", ret);
        delay(500);
        ...
        ret = ThebeI_GetDefaultDestAddr(&destaddr_lsb);
        Debug_out("ThebeI_GetDefaultDestAddr", ret);
        fprintf (stdout,"Destination address is 0x%x\n",destaddr_lsb);
        delay(500);

        ret = ThebeI_SetDefaultDestAddr(0x20,0x21);
        Debug_out("ThebeI_SetDefaultDestAddr to 0x20", ret);
        delay(500);
        ...
        ret = ThebeI_Transmit(payload,sizeof(payload));
        Debug_out("Transmit", ret);
        delay(500);

        /* Transmit "Hello, I am Amber" on channel 106 to address 0 0 */
        ret = ThebeI_Transmit_Extended(payload,sizeof(payload),106,0,0);
