...
static void TarvosI_test_function();
static void TarvosI_rx_test();
...
void *main_Thread(void *pArgs)
{
   ...
   TarvosI_rx_test();
#else
    /* function to test all functions of the TarvosI driver */
    TarvosI_test_function();
#endif
...
static void TarvosI_test_function()
{
    ...
    if(ret)
    {
        fprintf (stdout, COLOR_CYAN "TarvosI driver version %d.%d.%d\n" COLOR_RESET,driver_version[0],driver_version[1],driver_version[2]);
    }
    delay(500);

    /* initialize the module TarvosI */
    ret = TarvosI_Init(9600, TarvosI_PIN_RESET, RXcallback, AddressMode_0);
    Debug_out("TarvosI init", ret);
    ...
    if(ret)
    {
        ...
        ret = TarvosI_GetSerialNumber(serial_number);
        ...
        ret = TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, &data_length);
        ...
        data[0] = 0; /* 0dbm */
        data_length = 1;
        ret = TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, data_length);
        ...
        ret = TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_NUMRETRYS, numretries, &numretries_len);
        ...
        ret = TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_OP_MODE, opMode, &opMode_len);
        ...
        ret = TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_CFG_FLAGS, cfgFlags, &cfgFlags_len);
        Debug_out("GET: CFGFlags\n", ret);
        fprintf(stdout, "CFG Flags: %x %x\n", cfgFlags[0], cfgFlags[1]);
        delay(500);

        ret = TarvosI_SetVolatile_Channel(109);
        Debug_out("Set volatile Channel to 109", ret);
        delay(500);

        ret = TarvosI_SetVolatile_DestNetID(1);
        Debug_out("Set volatile DestNetID to 1", ret);
        delay(500);

        ret = TarvosI_SetVolatile_DestAddr(0x00,0x00);
        Debug_out("Set volatile DestAddr to 0", ret);
        delay(500);

        fprintf (stdout, "\n");

        ret = TarvosI_PinReset();
        Debug_out("PinReset", ret);
        delay(500);

        uint8_t channel = 0;
        ret = TarvosI_GetDefaultRFChannel(&channel);
        Debug_out("TarvosI_GetDefaultRFChannel", ret);
        fprintf (stdout,"RFChannel is %d\n",channel);
        delay(500);

        ret = TarvosI_SetDefaultRFChannel(102);
        Debug_out("TarvosI_SetDefaultRFChannel to 102", ret);
        delay(500);
        ...
        uint8_t srcnetid = 0;
        ret = TarvosI_GetSourceNetID(&srcnetid);
        Debug_out("TarvosI_GetSourceNetID", ret);
        fprintf (stdout,"Source net ID is %d\n",srcnetid);
        delay(500);

        ret = TarvosI_SetSourceNetID(17);
        Debug_out("TarvosI_SetSourceNetID to 17", ret);
        delay(500);
        ...
        uint8_t destnetid = 0;
        ret = TarvosI_GetDefaultDestNetID(&destnetid);
        Debug_out("TarvosI_GetDefaultDestNetID", ret);
        fprintf (stdout,"Destination net ID is 0x%x\n",destnetid);
        delay(500);

        ret = TarvosI_SetDefaultDestNetID(0x00);
        Debug_out("TarvosI_SetDefaultDestNetID to 0x12", ret);
        delay(500);
        ...
        ret = TarvosI_GetSourceAddr(&srcaddr_lsb);
        Debug_out("TarvosI_GetSourceAddr", ret);
        fprintf (stdout,"Source address is 0x%x\n",srcaddr_lsb);
        delay(500);

        ret = TarvosI_SetSourceAddr(0x13,0x14);
        Debug_out("TarvosI_SetSourceAddr to 0x13", ret);
        delay(500);
        ...
        ret = TarvosI_GetDefaultDestAddr(&destaddr_lsb);
        Debug_out("TarvosI_GetDefaultDestAddr", ret);
        fprintf (stdout,"Destination address is 0x%x\n",destaddr_lsb);
        delay(500);

        ret = TarvosI_SetDefaultDestAddr(0x20,0x21);
        Debug_out("TarvosI_SetDefaultDestAddr to 0x20", ret);
        delay(500);
        ...
        ret = TarvosI_Transmit(payload,sizeof(payload));
        Debug_out("Transmit", ret);
        delay(500);

        /* Transmit "Hello, I am Amber" on channel 106 to address 0 0 */
        ret = TarvosI_Transmit_Extended(payload,sizeof(payload),106,0,0);
        Debug_out("Transmit extended", ret);
        delay(500);
    }
    ret = TarvosI_Deinit();
    Debug_out("TarvosI deinit", ret);
}
