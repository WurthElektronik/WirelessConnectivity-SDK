...
static void TarvosII_test_function();
static void TarvosII_rx_test();
...
void *main_Thread(void *pArgs)
{
   ...
   TarvosII_rx_test();
#else
    /* function to test all functions of the TarvosII driver */
    TarvosII_test_function();
#endif
...
static void TarvosII_test_function()
{
    ...
    if(ret)
    {
        fprintf (stdout, COLOR_CYAN "TarvosII driver version %d.%d.%d\n" COLOR_RESET,driver_version[0],driver_version[1],driver_version[2]);
    }
    delay(500);

    /* initialize the module TarvosII */
    ret = TarvosII_Init(9600, TarvosII_PIN_RESET, RXcallback, AddressMode_0);
    Debug_out("TarvosII init", ret);

    if(ret)
    {
        ret = TarvosII_PinReset();
        ...
        ret = TarvosII_GetSerialNumber(serial_number);
        ...
        ret = TarvosII_Get(TarvosII_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, &data_length);
        ...
        data[0] = 0; /* 0dbm */
        data_length = 1;
        ret = TarvosII_Set(TarvosII_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, data_length);
        ...
        ret = TarvosII_Get(TarvosII_USERSETTING_MEMPOSITION_NUMRETRYS, numretries, &numretries_len);
        ...
        ret = TarvosII_Get(TarvosII_USERSETTING_MEMPOSITION_OP_MODE, opMode, &opMode_len);
        ...
        ret = TarvosII_Get(TarvosII_USERSETTING_MEMPOSITION_CFG_FLAGS, cfgFlags, &cfgFlags_len);
        ...
        ret = TarvosII_Get(TarvosII_USERSETTING_MEMPOSITION_UARTBAUDRATE, baudrate, &baudrate_len);
        ...
        ret = TarvosII_SetVolatile_TXPower(10);
        Debug_out("Set volatile TXpower to 10 dBm", ret);
        delay(500);

        ret = TarvosII_SetVolatile_Channel(110);
        Debug_out("Set volatile Channel to 110", ret);
        delay(500);

        ret = TarvosII_SetVolatile_DestNetID(1);
        Debug_out("Set volatile DestNetID to 1", ret);
        delay(500);

        ret = TarvosII_SetVolatile_DestAddr(0x00,0x00);
        ...
        int8_t txpower = 10;
        ret = TarvosII_GetDefaultTXPower(&txpower);
        Debug_out("TarvosII_GetDefaultTXPower", ret);
        fprintf (stdout,"TXPower is %d dBm\n",txpower);
        delay(500);

        ret = TarvosII_SetDefaultTXPower(2);
        Debug_out("TarvosII_SetDefaultTXPower to 2dBm", ret);
        delay(500);
        ...
        uint8_t channel = 0;
        ret = TarvosII_GetDefaultRFChannel(&channel);
        Debug_out("TarvosII_GetDefaultRFChannel", ret);
        fprintf (stdout,"RFChannel is %d\n",channel);
        delay(500);

        ret = TarvosII_SetDefaultRFChannel(101);
        Debug_out("TarvosII_SetDefaultRFChannel to 101", ret);
        delay(500);
        ...
        uint8_t srcnetid = 0;
        ret = TarvosII_GetSourceNetID(&srcnetid);
        Debug_out("TarvosII_GetSourceNetID", ret);
        fprintf (stdout,"Source net ID is %d\n",srcnetid);
        delay(500);

        ret = TarvosII_SetSourceNetID(17);
        Debug_out("TarvosII_SetSourceNetID to 17", ret);
        delay(500);
        ...
        uint8_t destnetid = 0;
        ret = TarvosII_GetDefaultDestNetID(&destnetid);
        Debug_out("TarvosII_GetDefaultDestNetID", ret);
        fprintf (stdout,"Destination net ID is 0x%x\n",destnetid);
        delay(500);

        ret = TarvosII_SetDefaultDestNetID(0x00);
        Debug_out("TarvosII_SetDefaultDestNetID to 0x12", ret);
        delay(500);
        ...
        ret = TarvosII_GetSourceAddr(&srcaddr_lsb);
        Debug_out("TarvosII_GetSourceAddr", ret);
        fprintf (stdout,"Source address is 0x%x\n",srcaddr_lsb);
        delay(500);

        ret = TarvosII_SetSourceAddr(0x13,0x14);
        Debug_out("TarvosII_SetSourceAddr to 0x13", ret);
        delay(500);
        ...
        ret = TarvosII_GetDefaultDestAddr(&destaddr_lsb);
        Debug_out("TarvosII_GetDefaultDestAddr", ret);
        fprintf (stdout,"Destination address is 0x%x\n",destaddr_lsb);
        delay(500);

        ret = TarvosII_SetDefaultDestAddr(0x20,0x21);
        Debug_out("TarvosII_SetDefaultDestAddr to 0x20", ret);
        delay(500);
        ...
        ret = TarvosII_Transmit(payload,sizeof(payload));
        Debug_out("Transmit", ret);
        delay(500);

        /* Transmit "Hello, I am Amber" on channel 106 to address 0 0 */
        ret = TarvosII_Transmit_Extended(payload,sizeof(payload),106,0,0);
