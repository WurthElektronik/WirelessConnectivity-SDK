...
static bool InitDriver(void(*RXcb)(uint8_t*,uint16_t,uint32_t,int8_t));
...
/*
 *Initialize the ThyoneI and driver
 *
 *input:
...
static bool InitDriver(void(*RXcb)(uint8_t*,uint16_t,uint32_t,int8_t))
{
    ...
    if(ThyoneI_GetFWVersion(firmware_version))
    {
 ...
 *       false otherwise
 */
bool ThyoneI_Init(int baudrate, int rp,  int wp, int bp, void(*RXcb)(uint8_t*,uint16_t,uint32_t,int8_t))
{
    if (false == InitSerial())
    {
        /* error */
