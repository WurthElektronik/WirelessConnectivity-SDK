...
#define CMD_STX 0x02
#define TarvosI_CMD_TYPE_REQ (0 << 6)
#define TarvosI_CMD_TYPE_CNF (1 << 6)
#define TarvosI_CMD_TYPE_IND (2 << 6)
#define TarvosI_CMD_TYPE_RSP (3 << 6)

#define TarvosI_CMD_DATA 0x00
#define TarvosI_CMD_DATA_REQ (TarvosI_CMD_DATA | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_DATA_CNF (TarvosI_CMD_DATA | TarvosI_CMD_TYPE_CNF)
#define TarvosI_CMD_REPEAT_IND (TarvosI_CMD_DATA | TarvosI_CMD_TYPE_IND)

#define TarvosI_CMD_DATAEX 0x01
#define TarvosI_CMD_DATAEX_REQ (TarvosI_CMD_DATAEX | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_DATAEX_IND (TarvosI_CMD_DATAEX | TarvosI_CMD_TYPE_IND)

#define TarvosI_CMD_SETMODE 0x04
#define TarvosI_CMD_SETMODE_CNF (TarvosI_CMD_SETMODE | TarvosI_CMD_TYPE_CNF)
#define TarvosI_CMD_SETMODE_IND (TarvosI_CMD_SETMODE | TarvosI_CMD_TYPE_IND)

#define TarvosI_CMD_RESET 0x05
#define TarvosI_CMD_RESET_REQ (TarvosI_CMD_RESET | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_RESET_CNF (TarvosI_CMD_RESET | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_SET_CHANNEL 0x06
#define TarvosI_CMD_SET_CHANNEL_REQ (TarvosI_CMD_SET_CHANNEL | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_SET_CHANNEL_CNF (TarvosI_CMD_SET_CHANNEL | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_SET_DESTNETID 0x07
#define TarvosI_CMD_SET_DESTNETID_REQ (TarvosI_CMD_SET_DESTNETID | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_SET_DESTNETID_CNF (TarvosI_CMD_SET_DESTNETID | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_SET_DESTADDR 0x08
#define TarvosI_CMD_SET_DESTADDR_REQ (TarvosI_CMD_SET_DESTADDR | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_SET_DESTADDR_CNF (TarvosI_CMD_SET_DESTADDR | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_SET 0x09
#define TarvosI_CMD_SET_REQ (TarvosI_CMD_SET | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_SET_CNF (TarvosI_CMD_SET | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_GET 0x0A
#define TarvosI_CMD_GET_REQ (TarvosI_CMD_GET | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_GET_CNF (TarvosI_CMD_GET | TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_GET_SERIALNO 0x0B
#define TarvosI_CMD_GET_SERIALNO_REQ (TarvosI_CMD_GET_SERIALNO | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_GET_SERIALNO_CNF (TarvosI_CMD_GET_SERIALNO| TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_GET_FWRELEASE 0x0C
#define TarvosI_CMD_GET_FWRELEASE_REQ (TarvosI_CMD_GET_FWRELEASE | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_GET_FWRELEASE_CNF (TarvosI_CMD_GET_FWRELEASE| TarvosI_CMD_TYPE_CNF)

#define TarvosI_CMD_RSSI 0x0D
#define TarvosI_CMD_RSSI_REQ (TarvosI_CMD_RSSI | TarvosI_CMD_TYPE_REQ)
#define TarvosI_CMD_RSSI_CNF (TarvosI_CMD_RSSI | TarvosI_CMD_TYPE_CNF)

/* type used to check the response, when a command was sent to the TarvosI */
...
typedef enum TarvosI_OperationMode_t
{
    TarvosI_OperationMode_Transparent = 0x00, /* invalid for operation with this driver */
    TarvosI_OperationMode_Command = 0x10,
}
TarvosI_OperationMode_t;
...
static bool Wait4CNF(int max_time_ms, uint8_t expectedCmdConfirmation, CMD_Status_t expectedStatus, bool reset_confirmstate);   /* wait for response, when a command was sent to the TarvosI */
static bool FillChecksum(uint8_t* array, uint8_t length);                                                                       /* add the CS needed to finalize the command */
static bool InitDriver(void(*RXcb)(uint8_t*,uint8_t,uint8_t,uint8_t,uint8_t,int8_t), TarvosI_AddressMode_t addrmode);
...
TarvosI_AddressMode_t addressmode = AddressMode_0;                        /* initial address mode */
...
/*
 *Initialize the TarvosI and driver
 *
 *input:
 * -RXcb:      RX callback function
 * -addrmode:  address mode of the TarvosI
 *
...
static bool InitDriver(void(*RXcb)(uint8_t*,uint8_t,uint8_t,uint8_t,uint8_t,int8_t), TarvosI_AddressMode_t addrmode)
{
    ...
    if(TarvosI_GetFirmwareVersion(response))
    {
...
static void HandleRxPacket(uint8_t*RxBuffer)
{
    ...
    switch (RxPacket.Cmd)
    {
    case TarvosI_CMD_SETMODE_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == TarvosI_OperationMode_Command)
        {
    ...
    case TarvosI_CMD_GET_SERIALNO_CNF:
    {
    ...
    case TarvosI_CMD_GET_FWRELEASE_CNF:
    {
    ...
    case TarvosI_CMD_DATAEX_IND:
    {
        ...
        if (RXcallback != NULL)
        {
            ...
            case AddressMode_0:
            {
                RXcallback(&RxPacket.Data[0], RxPacket.Length - 1, TarvosI_BROADCASTADDRESS, TarvosI_BROADCASTADDRESS, TarvosI_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;

            case AddressMode_1:
            {
                RXcallback(&RxPacket.Data[1], RxPacket.Length - 2, TarvosI_BROADCASTADDRESS, RxPacket.Data[0], TarvosI_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;

            case AddressMode_2:
            {
                RXcallback(&RxPacket.Data[2], RxPacket.Length - 3, RxPacket.Data[0], RxPacket.Data[1], TarvosI_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;
    ...
    case TarvosI_CMD_SET_CHANNEL_CNF:
    {
    ...
    case TarvosI_CMD_SET_DESTADDR_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == 0x00)
        {
    ...
    case TarvosI_CMD_SET_DESTNETID_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == 0x00)
        {
...
/* function that waits for the return value of TarvosI (*_CNF), when a command (*_REQ) was sent before */
...
/*
 *Initialize the TarvosI interface for serial interface
 *
 *input:
 ...
 * -addrmode:  address mode of the TarvosI
 *
 *Caution: the parameters baudrate and addrmode must match the configured UserSettings of the TarvosI
 ...
 *          *this parameter can be updated to the correct value (used in TarvosI_Init function) as soon as no RF packet transmission or reception was performed
 *
 *return true if initialization succeeded
 *       false otherwise
 */
bool TarvosI_Init(int baudrate, int rp, void(*RXcb)(uint8_t*,uint8_t,uint8_t,uint8_t,uint8_t,int8_t), TarvosI_AddressMode_t addrmode)
{
    if (false == InitPin(rp))
    {
        /* error */
...
/*
 *Deinitialize the TarvosI interface
 *
...
/*
 *Reset the TarvosI by pin
 *
 *return true if reset succeeded
 *       false otherwise
 */
bool TarvosI_PinReset()
{
    /* set to output mode */
    ...
    /* wait for cnf */
    return Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SETMODE_CNF, CMD_Status_Success, true);
}

/*
 *Reset the TarvosI by command
 *
 *return true if reset succeeded
 *       false otherwise
 */
bool TarvosI_Reset()
{
    ...
    uint8_t CMD_ARRAY[4];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_RESET_REQ;
    CMD_ARRAY[2] = 0x00;
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SETMODE_CNF, CMD_Status_Success, true);
    }
    return ret;
}

/*
 *Request the current TarvosI settings
 *
 *input:
 ...
 *       false otherwise
 */
bool TarvosI_Get(TarvosI_UserSettings_t us, uint8_t* response, uint8_t* response_length)
{
    bool ret = false;
    ...
    uint8_t CMD_ARRAY[6];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_GET_REQ;
    CMD_ARRAY[2] = 0x02;
    CMD_ARRAY[3] = us;
    ...
    case(TarvosI_USERSETTING_MEMPOSITION_NUMRETRYS):
    case(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTNETID):
    case(TarvosI_USERSETTING_MEMPOSITION_SOURCENETID):
    case(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER):
    case(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFCHANNEL):
    case(TarvosI_USERSETTING_MEMPOSITION_OP_MODE):
    {
        CMD_ARRAY[4] = 0x01;
    }
    break;
    ...
    case(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTADDR):
    case(TarvosI_USERSETTING_MEMPOSITION_SOURCEADDR):
    case(TarvosI_USERSETTING_MEMPOSITION_CFG_FLAGS):
    {
        CMD_ARRAY[4] = 0x02;
    }
    break;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        if (Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_GET_CNF, CMD_Status_Success, true))
        {
...
/*
 *Request multiple of the current TarvosI settings
 *
 *input:
 ...
 *       false otherwise
 */
bool TarvosI_GetMultiple(uint8_t startAddress, uint8_t lengthToRead, uint8_t *response, uint8_t *response_length)
{
    bool ret = false;
 ...
 *       false otherwise
 */
bool TarvosI_Set(TarvosI_UserSettings_t us, uint8_t* value, uint8_t length)
{
    bool ret = false;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_SET_REQ;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SET_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool TarvosI_GetFirmwareVersion(uint8_t* fw)
{
    uint8_t CMD_ARRAY[4];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_GET_FWRELEASE_REQ;
    CMD_ARRAY[2] = 0;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        if (Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_GET_FWRELEASE_CNF, CMD_Status_Success, true))
        {
 ...
 *       false otherwise
 */
bool TarvosI_GetSerialNumber(uint8_t* sn)
{
    uint8_t CMD_ARRAY[4];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_GET_SERIALNO_REQ;
    CMD_ARRAY[2] = 0;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        if (Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_GET_SERIALNO_CNF, CMD_Status_Success, true))
        {
 ...
 *       false otherwise
 */
bool TarvosI_GetDefaultDestAddr(uint8_t* destaddr_lsb)
{
    uint8_t length;

    if(TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTADDR, destaddr_lsb, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool TarvosI_GetDefaultDestNetID(uint8_t* destnetid)
{

    uint8_t length;

    if(TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTNETID, destnetid, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool TarvosI_GetSourceAddr(uint8_t* srcaddr_lsb)
{

    uint8_t length;

    if(TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_SOURCEADDR, srcaddr_lsb, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool TarvosI_GetSourceNetID(uint8_t* srcnetid)
{

    uint8_t length;

    if(TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_SOURCENETID, srcnetid, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool TarvosI_GetDefaultRFChannel(uint8_t* channel)
{
    uint8_t length;

    if(TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFCHANNEL, channel, &length))
    {
        return true;
    }
    else
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use TarvosI_SetVolatile_TXPower for frequent adaption of the TX power
 *
 *return true if request succeeded
 *       false otherwise
 */
bool TarvosI_SetDefaultTXPower(int8_t txpower)
{
    ...
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, (uint8_t*)&txpower, 1);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use TarvosI_SetVolatile_DestAddr for frequent adaption of the destination address
 *
 *return true if request succeeded
 *       false otherwise
 */
bool TarvosI_SetDefaultDestAddr(uint8_t destaddr_lsb, uint8_t destaddr_msb)
{
    ...
    help[0] = destaddr_lsb;
    help[1] = destaddr_msb;
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTADDR, help, 2);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use TarvosI_SetVolatile_DestNetID for frequent adaption of the destination net id
 *
 *return true if request succeeded
 *       false otherwise
 */
bool TarvosI_SetDefaultDestNetID(uint8_t destnetid)
{
    ...
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_DEFAULTDESTNETID, &destnetid, 1);
}
 ...
 *       false otherwise
 */
bool TarvosI_SetSourceAddr(uint8_t srcaddr_lsb, uint8_t srcaddr_msb)
{
    /*check for invalid address */
    ...
    help[0] = srcaddr_lsb;
    help[1] = srcaddr_msb;
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_SOURCEADDR, help, 2);
}
 ...
 *       false otherwise
 */
bool TarvosI_SetSourceNetID(uint8_t srcnetid)
{
    ...
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_SOURCENETID, &srcnetid, 1);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use TarvosI_SetVolatile_Channel for frequent adaption of the channel
 *
 *return true if request succeeded
 *       false otherwise
 */
bool TarvosI_SetDefaultRFChannel(uint8_t channel)
{
    /* check for invalid channel */
    if((channel<102)||(channel>110))
    {
        /* invalid channel */
        return false;
    }
    return TarvosI_Set(TarvosI_USERSETTING_MEMPOSITION_DEFAULTRFCHANNEL, &channel, 1);
}
 ...
 *       false otherwise
 */
bool TarvosI_SetVolatile_Channel(uint8_t channel)
{
    bool ret = false;
    ...
    uint8_t CMD_ARRAY[5];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_SET_CHANNEL_REQ;
    CMD_ARRAY[2] = 0x01;
    CMD_ARRAY[3] = channel;
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        */
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SET_CHANNEL_CNF, CMD_Status_Success, true);
 ...
 *       false otherwise
 */
bool TarvosI_SetVolatile_DestNetID(uint8_t destnetid)
{
    bool ret = false;
    if(destnetid > 255)
    {
    ...
    uint8_t CMD_ARRAY[5];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_SET_DESTNETID_REQ;
    CMD_ARRAY[2] = 0x01;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SET_DESTNETID_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool TarvosI_SetVolatile_DestAddr(uint8_t destaddr_lsb, uint8_t destaddr_msb)
{
    bool ret = false;
    ...
    {
        CMD_ARRAY[0] = CMD_STX;
        CMD_ARRAY[1] = TarvosI_CMD_SET_DESTADDR_REQ;
        CMD_ARRAY[2] = 0x01;
    ...
    if(ret == true)
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_SET_DESTADDR_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool TarvosI_Transmit(uint8_t* payload, uint8_t length)
{
    bool ret = false;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_DATA_REQ;
    CMD_ARRAY[2] = length;
    memcpy(&CMD_ARRAY[3],payload,length);
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        /* check for value if numretries - if value == 0 no ack is expected */
        TarvosI_Get(TarvosI_USERSETTING_MEMPOSITION_NUMRETRYS, &numretries, &responseLength);
        ...
        if(numretries > 0)
        {
            ...
            ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_DATA_CNF, CMD_Status_Success, true);
        }
        else
        {
            /* no ack will be requested from receiver module - In this case CMD_Status_Failed is sent immediately after transmission */
            ret = Wait4CNF(CMD_WAIT_TIME, TarvosI_CMD_DATA_CNF, CMD_Status_Failed, true);
        }
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool TarvosI_Transmit_Extended(uint8_t* payload, uint8_t length, uint8_t channel, uint8_t dest_network_id, uint8_t dest_address_lsb)
{
    bool ret = false;
    uint8_t numretries = -1;
    uint8_t responseLength = 1;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = TarvosI_CMD_DATAEX_REQ;
 ...
 *       false otherwise
*/
bool TarvosI_Configure(TarvosI_Configuration_t* config, uint8_t config_length)
{
    int i = 0;
    uint8_t help_length;
    ...
    {
        /* read current value */
        if(false == TarvosI_Get(config[i].usersetting, help, &help_length))
        {
            /* error */
            return false;
        }
        ...
        if(memcmp(help,config[i].value,config[i].value_length) != 0)
        {
            ...
            if(false == TarvosI_Set(config[i].usersetting, config[i].value, config[i].value_length))
            {
                /* error */
                return false;
            }
