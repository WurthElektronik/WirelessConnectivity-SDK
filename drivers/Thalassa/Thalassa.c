...
#define CMD_STX 0x02
#define Thalassa_CMD_TYPE_REQ (0 << 6)
#define Thalassa_CMD_TYPE_CNF (1 << 6)
#define Thalassa_CMD_TYPE_IND (2 << 6)
#define Thalassa_CMD_TYPE_RSP (3 << 6)

#define Thalassa_CMD_DATA 0x00
#define Thalassa_CMD_DATA_REQ (Thalassa_CMD_DATA | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_DATA_CNF (Thalassa_CMD_DATA | Thalassa_CMD_TYPE_CNF)
#define Thalassa_CMD_REPEAT_IND (Thalassa_CMD_DATA | Thalassa_CMD_TYPE_IND)

#define Thalassa_CMD_DATAEX 0x01
#define Thalassa_CMD_DATAEX_REQ (Thalassa_CMD_DATAEX | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_DATAEX_IND (Thalassa_CMD_DATAEX | Thalassa_CMD_TYPE_IND)

#define Thalassa_CMD_SETMODE 0x04
#define Thalassa_CMD_SETMODE_CNF (Thalassa_CMD_SETMODE | Thalassa_CMD_TYPE_CNF)
#define Thalassa_CMD_SETMODE_IND (Thalassa_CMD_SETMODE | Thalassa_CMD_TYPE_IND)

#define Thalassa_CMD_RESET 0x05
#define Thalassa_CMD_RESET_REQ (Thalassa_CMD_RESET | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_RESET_CNF (Thalassa_CMD_RESET | Thalassa_CMD_TYPE_CNF)
#define Thalassa_CMD_RESET_IND (Thalassa_CMD_RESET | Thalassa_CMD_TYPE_IND)

#define Thalassa_CMD_SET_CHANNEL 0x06
#define Thalassa_CMD_SET_CHANNEL_REQ (Thalassa_CMD_SET_CHANNEL | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_SET_CHANNEL_CNF (Thalassa_CMD_SET_CHANNEL | Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_SET_DESTNETID 0x07
#define Thalassa_CMD_SET_DESTNETID_REQ (Thalassa_CMD_SET_DESTNETID | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_SET_DESTNETID_CNF (Thalassa_CMD_SET_DESTNETID | Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_SET_DESTADDR 0x08
#define Thalassa_CMD_SET_DESTADDR_REQ (Thalassa_CMD_SET_DESTADDR | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_SET_DESTADDR_CNF (Thalassa_CMD_SET_DESTADDR | Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_SET 0x09
#define Thalassa_CMD_SET_REQ (Thalassa_CMD_SET | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_SET_CNF (Thalassa_CMD_SET | Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_GET 0x0A
#define Thalassa_CMD_GET_REQ (Thalassa_CMD_GET | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_GET_CNF (Thalassa_CMD_GET | Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_GET_SERIALNO 0x0B
#define Thalassa_CMD_GET_SERIALNO_REQ (Thalassa_CMD_GET_SERIALNO | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_GET_SERIALNO_CNF (Thalassa_CMD_GET_SERIALNO| Thalassa_CMD_TYPE_CNF)

#define Thalassa_CMD_RSSI 0x0D
#define Thalassa_CMD_RSSI_REQ (Thalassa_CMD_RSSI | Thalassa_CMD_TYPE_REQ)
#define Thalassa_CMD_RSSI_CNF (Thalassa_CMD_RSSI | Thalassa_CMD_TYPE_CNF)

/* type used to check the response, when a command was sent to the Thalassa */
...
typedef enum Thalassa_OperationMode_t
{
    Thalassa_OperationMode_Transparent = 0x00, /* invalid for operation with this driver */
    Thalassa_OperationMode_Command = 0x10,
}
Thalassa_OperationMode_t;
...
static bool Wait4CNF(int max_time_ms, uint8_t expectedCmdConfirmation, CMD_Status_t expectedStatus, bool reset_confirmstate);   /* wait for response, when a command was sent to the Thalassa */
static bool FillChecksum(uint8_t* array, uint8_t length);                                         /* add the CS needed to finalize the command */
static bool InitDriver(void(*RXcb)(uint8_t*,uint8_t,uint8_t,uint8_t,uint8_t,int8_t), Thalassa_AddressMode_t addrmode);
...
Thalassa_AddressMode_t addressmode = AddressMode_0;                        /* initial address mode */
...
/*
 *Initialize the Thalassa and driver
 *
 *input:
 * -RXcb:      RX callback function
 * -addrmode:  address mode of the Thalassa
 *
...
static void HandleRxPacket(uint8_t*RxBuffer)
{
    ...
    switch (RxPacket.Cmd)
    {
    case Thalassa_CMD_SETMODE_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == Thalassa_OperationMode_Command)
        {
    ...
    case Thalassa_CMD_RESET_IND:
    {
        cmdConfirmation.status = CMD_Status_Success;
        cmdConfirmation.cmd = Thalassa_CMD_RESET_IND;
    }
    break;

    case Thalassa_CMD_DATA_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == 0x00)
        {
    ...
    case Thalassa_CMD_GET_SERIALNO_CNF:
    {
    ...
    case Thalassa_CMD_DATAEX_IND:
    {
        ...
        if (RXcallback != NULL)
        {
            ...
            case AddressMode_0:
            {
                RXcallback(&RxPacket.Data[0], RxPacket.Length - 1, Thalassa_BROADCASTADDRESS, Thalassa_BROADCASTADDRESS, Thalassa_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;

            case AddressMode_1:
            {
                RXcallback(&RxPacket.Data[1], RxPacket.Length - 2, Thalassa_BROADCASTADDRESS, RxPacket.Data[0], Thalassa_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;

            case AddressMode_2:
            {
                RXcallback(&RxPacket.Data[2], RxPacket.Length - 3, RxPacket.Data[0], RxPacket.Data[1], Thalassa_BROADCASTADDRESS, CalculateRSSIValue(RxPacket.Data[RxPacket.Length-1]));
            }
            break;
    ...
    case Thalassa_CMD_SET_CHANNEL_CNF:
    {
    ...
    case Thalassa_CMD_SET_DESTADDR_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == 0x00)
        {
    ...
    case Thalassa_CMD_SET_DESTNETID_CNF:
    {
        /* check whether the module returns success */
        if (RxPacket.Data[0] == 0x00)
        {
...
/* function that waits for the return value of Thalassa (*_CNF), when a command (*_REQ) was sent before */
 ...
 **************************************/
/*
 *Initialize the Thalassa interface
 *
 *input:
 ...
 * -addrmode:  address mode of the Thalassa
 *
 *Caution: the parameters baudrate and addrmode must match the configured UserSettings of the Thalassa
 ...
 *          *this parameter can be updated to the correct value (used in Thalassa_Init function) as soon as no RF packet transmission or reception was performed
 *
 *return true if initialization succeeded
 *       false otherwise
 */
bool Thalassa_Init(int baudrate, int rp, void(*RXcb)(uint8_t*,uint8_t,uint8_t,uint8_t,uint8_t,int8_t), Thalassa_AddressMode_t addrmode)
{
...
/*
 *Deinitialize the Thalassa interface
 *
 *return true if deinitialization succeeded
 *       false otherwise
 */
bool Thalassa_Deinit()
{
...
/*
 *Reset the Thalassa by pin
 *
 *return true if reset succeeded
 *       false otherwise
 */
bool Thalassa_PinReset()
{
    /* set to output mode */
    ...
    /* wait for cnf */
    return Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_SETMODE_CNF, CMD_Status_Success, true);
}

/*
 *Reset the Thalassa by command
 *
 *return true if reset succeeded
 *       false otherwise
 */
bool Thalassa_Reset()
{
    ...
    uint8_t CMD_ARRAY[4];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_RESET_REQ;
    CMD_ARRAY[2] = 0x00;
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_RESET_CNF, CMD_Status_Success, true);
    }
    return ret;
}

/*
 *Request the current Thalassa settings
 *
 *input:
 ...
 *       false otherwise
 */
bool Thalassa_Get(Thalassa_UserSettings_t us, uint8_t* response, uint8_t* response_length)
{
    bool ret = false;
    ...
    uint8_t CMD_ARRAY[6];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_GET_REQ;
    CMD_ARRAY[2] = 0x02;
    CMD_ARRAY[3] = us;
    ...
    case(Thalassa_USERSETTING_MEMPOSITION_NUMRETRYS):
    case(Thalassa_USERSETTING_MEMPOSITION_DESTNETID):
    case(Thalassa_USERSETTING_MEMPOSITION_DESTADDRLSB):
    case(Thalassa_USERSETTING_MEMPOSITION_SOURCENETID):
    case(Thalassa_USERSETTING_MEMPOSITION_SOURCEADDRLSB):
    case(Thalassa_USERSETTING_MEMPOSITION_PAPOWER):
    case(Thalassa_USERSETTING_MEMPOSITION_DEFAULTCHANNEL):
    case(Thalassa_USERSETTING_MEMPOSITION_OP_MODE):
    {
        CMD_ARRAY[4] = 0x01;
    }
    break;
    ...
    case(Thalassa_USERSETTING_MEMPOSITION_CFG_FLAGS):
    {
        CMD_ARRAY[4] = 0x02;
    }
    break;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        if (Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_GET_CNF, CMD_Status_Success, true))
        {
 ...
 *       false otherwise
 */
bool Thalassa_GetMultiple(uint8_t startAddress, uint8_t lengthToRead, uint8_t *response, uint8_t *response_length)
{
    bool ret = false;
...
/*
 *Set a special Thalassa setting
 *
 *input:
 ...
 *       false otherwise
 */
bool Thalassa_Set(Thalassa_UserSettings_t us, uint8_t* value, uint8_t length)
{
    bool ret = false;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_SET_REQ;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_SET_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool Thalassa_GetSerialNumber(uint8_t* sn)
{
    uint8_t CMD_ARRAY[4];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_GET_SERIALNO_REQ;
    CMD_ARRAY[2] = 0;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        if (Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_GET_SERIALNO_CNF, CMD_Status_Success, true))
        {
 ...
 *       false otherwise
 */
bool Thalassa_GetDefaultTXPower(int8_t* txpower)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_PAPOWER, (uint8_t*)txpower, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool Thalassa_GetDefaultDestAddr(uint8_t* destaddr_lsb)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_DESTADDRLSB, destaddr_lsb, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool Thalassa_GetDefaultDestNetID(uint8_t* destnetid)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_DESTNETID, destnetid, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool Thalassa_GetSourceAddr(uint8_t* srcaddr_lsb)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_SOURCEADDRLSB, srcaddr_lsb, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool Thalassa_GetSourceNetID(uint8_t* srcnetid)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_SOURCENETID, srcnetid, &length))
    {
        return true;
    }
    else
 ...
 *       false otherwise
 */
bool Thalassa_GetDefaultRFChannel(uint8_t* channel)
{
    uint8_t length;

    if(Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_DEFAULTCHANNEL, channel, &length))
    {
        return true;
    }
    else
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use Thalassa_SetVolatile_TXPower for frequent adaption of the TX power
 *
 *return true if request succeeded
 *       false otherwise
 */
bool Thalassa_SetDefaultTXPower(int8_t txpower)
{
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_PAPOWER, (uint8_t*)&txpower, 1);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use Thalassa_SetVolatile_DestAddr for frequent adaption of the destination address
 *
 *return true if request succeeded
 *       false otherwise
 */
bool Thalassa_SetDefaultDestAddr(uint8_t destaddr_lsb)
{
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_DESTADDRLSB, &destaddr_lsb, 1);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use Thalassa_SetVolatile_DestNetID for frequent adaption of the destination net id
 *
 *return true if request succeeded
 *       false otherwise
 */
bool Thalassa_SetDefaultDestNetID(uint8_t destnetid)
{
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_DESTNETID, &destnetid, 1);
}
 ...
 *       false otherwise
 */
bool Thalassa_SetSourceAddr(uint8_t srcaddr_lsb)
{
    /*check for invalid address */
    if(srcaddr_lsb > 255)
    {
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_SOURCEADDRLSB, &srcaddr_lsb, 1);
}
 ...
 *       false otherwise
 */
bool Thalassa_SetSourceNetID(uint8_t srcnetid)
{
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_SOURCENETID, &srcnetid, 1);
}
 ...
 *note: reset the module after the adaption of the setting such that it can take effect
 *note: use this function only in rare case, since flash can be updated only a limited number times
 *note: use Thalassa_SetVolatile_RFChannel for frequent adaption of the channel
 *
 *return true if request succeeded
 *       false otherwise
 */
bool Thalassa_SetDefaultRFChannel(uint8_t channel)
{
    ...
    return Thalassa_Set(Thalassa_USERSETTING_MEMPOSITION_DEFAULTCHANNEL, &channel, 1);
}
 ...
 *       false otherwise
 */
bool Thalassa_SetVolatile_RFChannel(uint8_t channel)
{
    bool ret = false;
    ...
    uint8_t CMD_ARRAY[5];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_SET_CHANNEL_REQ;
    CMD_ARRAY[2] = 0x01;
    CMD_ARRAY[3] = channel;
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        */
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_SET_CHANNEL_CNF, CMD_Status_Success, true);
 ...
 *       false otherwise
 */
bool Thalassa_SetVolatile_DestNetID(uint8_t destnetid)
{
    bool ret = false;
    if(destnetid > 255)
    {
    ...
    uint8_t CMD_ARRAY[5];
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_SET_DESTNETID_REQ;
    CMD_ARRAY[2] = 0x01;
    ...
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_SET_DESTNETID_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool Thalassa_SetVolatile_DestAddr(uint8_t destaddr_lsb)
{
    bool ret = false;
    ...
    {
        CMD_ARRAY[0] = CMD_STX;
        CMD_ARRAY[1] = Thalassa_CMD_SET_DESTADDR_REQ;
        CMD_ARRAY[2] = 0x01;
    ...
    if(ret == true)
    {
        ...
        /* wait for cnf */
        ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_SET_DESTADDR_CNF, CMD_Status_Success, true);
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool Thalassa_Transmit(uint8_t* payload, uint8_t length)
{
    bool ret = false;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_DATA_REQ;
    CMD_ARRAY[2] = length;
    memcpy(&CMD_ARRAY[3],payload,length);
    if(FillChecksum(CMD_ARRAY,sizeof(CMD_ARRAY)))
    {
        /* check for value if numreties - if value == 0 no ack is expected */
        Thalassa_Get(Thalassa_USERSETTING_MEMPOSITION_NUMRETRYS, &numretries, &responseLength);
        ...
        if(numretries > 0)
        {
            ...
            ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_DATA_CNF, CMD_Status_Success, true);
        }
        else
        {
            /* no ack will be requested from receiver module - In this case CMD_Status_Failed is sent immediately after transmission */
            ret = Wait4CNF(CMD_WAIT_TIME, Thalassa_CMD_DATA_CNF, CMD_Status_Failed, true);
        }
    }
    return ret;
}
 ...
 *       false otherwise
 */
bool Thalassa_Transmit_Extended(uint8_t* payload, uint8_t length, uint8_t channel, uint8_t dest_network_id, uint8_t dest_address_lsb)
{
    bool ret = false;
    uint8_t numretries = -1;
    uint8_t responseLength = 1;
    ...
    CMD_ARRAY[0] = CMD_STX;
    CMD_ARRAY[1] = Thalassa_CMD_DATAEX_REQ;
 ...
 *       false otherwise
*/
bool Thalassa_Configure(Thalassa_Configuration_t* config, uint8_t config_length)
{
    int i = 0;
    uint8_t help_length;
    ...
    {
        /* read current value */
        if(false == Thalassa_Get(config[i].usersetting, help, &help_length))
        {
            /* error */
            return false;
        }
        ...
        if(memcmp(help,config[i].value,config[i].value_length) != 0)
        {
            ...
            if(false == Thalassa_Set(config[i].usersetting, config[i].value, config[i].value_length))
            {
                /* error */
                return false;
            }
