...
#include "../drivers/TelestoII/TelestoII.h"
#include "../drivers/WE-common.h"
#include "../drivers/global/global.h"


static void TelestoII_test_function();
static void TelestoII_rx_test();
...
void *main_Thread(void *pArgs)
{
   ...
   TelestoII_rx_test();
#else
    /* function to test all functions of the TelestoII driver */
    TelestoII_test_function();
#endif
...
static void TelestoII_test_function()
{
    ...
    if(ret)
    {
        fprintf (stdout, COLOR_CYAN "TelestoII driver version %d.%d.%d\n" COLOR_RESET,driver_version[0],driver_version[1],driver_version[2]);
    }
    delay(500);

    /* initialize the module TelestoII */
    ret = TelestoII_Init(9600, TelestoII_PIN_RESET, RXcallback, AddressMode_0);
    Debug_out("TelestoII init", ret);

    if(ret)
    {
        ret = TelestoII_PinReset();
        ...
        ret = TelestoII_GetSerialNumber(serial_number);
        ...
        ret = TelestoII_Get(TelestoII_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, &data_length);
        ...
        data[0] = 0; /* 0dbm */
        data_length = 1;
        ret = TelestoII_Set(TelestoII_USERSETTING_MEMPOSITION_DEFAULTRFTXPOWER, data, data_length);
        ...
        ret = TelestoII_Get(TelestoII_USERSETTING_MEMPOSITION_NUMRETRYS, numretries, &numretries_len);
        ...
        ret = TelestoII_Get(TelestoII_USERSETTING_MEMPOSITION_OP_MODE, opMode, &opMode_len);
        ...
        ret = TelestoII_Get(TelestoII_USERSETTING_MEMPOSITION_CFG_FLAGS, cfgFlags, &cfgFlags_len);
        ...
        ret = TelestoII_Get(TelestoII_USERSETTING_MEMPOSITION_UARTBAUDRATE, baudrate, &baudrate_len);
        ...
        ret = TelestoII_SetVolatile_TXPower(10);
        Debug_out("Set volatile TXpower to 10 dBm", ret);
        delay(500);

        ret = TelestoII_SetVolatile_DestNetID(1);
        Debug_out("Set volatile DestNetID to 1", ret);
        delay(500);

        ret = TelestoII_SetVolatile_DestAddr(0x00,0x00);
        ...
        int8_t txpower = 10;
        ret = TelestoII_GetDefaultTXPower(&txpower);
        Debug_out("TelestoII_GetDefaultTXPower", ret);
        fprintf (stdout,"TXPower is %d dBm\n",txpower);
        delay(500);

        ret = TelestoII_SetDefaultTXPower(2);
        Debug_out("TelestoII_SetDefaultTXPower to 2dBm", ret);
        delay(500);
        ...
        uint8_t srcnetid = 0;
        ret = TelestoII_GetSourceNetID(&srcnetid);
        Debug_out("TelestoII_GetSourceNetID", ret);
        fprintf (stdout,"Source net ID is %d\n",srcnetid);
        delay(500);

        ret = TelestoII_SetSourceNetID(17);
        Debug_out("TelestoII_SetSourceNetID to 17", ret);
        delay(500);
        ...
        uint8_t destnetid = 0;
        ret = TelestoII_GetDefaultDestNetID(&destnetid);
        Debug_out("TelestoII_GetDefaultDestNetID", ret);
        fprintf (stdout,"Destination net ID is 0x%x\n",destnetid);
        delay(500);

        ret = TelestoII_SetDefaultDestNetID(0x00);
        Debug_out("TelestoII_SetDefaultDestNetID to 0x12", ret);
        delay(500);
        ...
        ret = TelestoII_GetSourceAddr(&srcaddr_lsb);
        Debug_out("TelestoII_GetSourceAddr", ret);
        fprintf (stdout,"Source address is 0x%x\n",srcaddr_lsb);
        delay(500);

        ret = TelestoII_SetSourceAddr(0x13,0x14);
        Debug_out("TelestoII_SetSourceAddr to 0x13", ret);
        delay(500);
        ...
        ret = TelestoII_GetDefaultDestAddr(&destaddr_lsb);
        Debug_out("TelestoII_GetDefaultDestAddr", ret);
        fprintf (stdout,"Destination address is 0x%x\n",destaddr_lsb);
        delay(500);

        ret = TelestoII_SetDefaultDestAddr(0x20,0x21);
        Debug_out("TelestoII_SetDefaultDestAddr to 0x20", ret);
        delay(500);
        ...
        ret = TelestoII_Transmit(payload,sizeof(payload));
        Debug_out("Transmit", ret);
        delay(500);

        /* Transmit "Hello, I am Amber" to address 0 0 */
        ret = TelestoII_Transmit_Extended(payload,sizeof(payload),0,0);
        Debug_out("Transmit extended", ret);
        delay(500);

    }

    ret = TelestoII_Deinit();
    Debug_out("TelestoII deinit", ret);
}
