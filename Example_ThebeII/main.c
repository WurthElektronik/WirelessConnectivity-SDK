...
static void ThebeII_test_function(void);
...
static void RX_test()
{
    ...
    ret = ThebeII_Init(115200, ThebeII_PIN_RESET, ThebeII_PIN_WAKEUP, ThebeII_PIN_BOOT, RXcallback, AddressMode_0);
    Debug_out("ThebeII_Init", ret);
...
/* test function to demonstrate the usage of the ThebeII functions */
static void ThebeII_test_function()
{
    bool ret = false;
    ...
    if(ret)
    {
        ...
        ret = ThebeII_GetSerialNumber(serial_number);
        ...
        ret = ThebeII_Shutdown();
        Debug_out("Shutdown", ret);
        delay(500);

        ret = ThebeII_PinWakeup();
        ...
        ret = ThebeII_Get(ThebeII_CMD_SETGET_OPTION_DEFAULTRFTXPOWER, data, &data_length);
        ...
        data[0] = 18; /* 18dbm */
        data_length = 1;
        ret = ThebeII_Set(ThebeII_CMD_SETGET_OPTION_DEFAULTRFTXPOWER, data, data_length);
        ...
        ret = ThebeII_SetVolatile_TXPower(12);
        Debug_out("Set volatile TXpower to 12 dBm", ret);
        delay(500);

        ret = ThebeII_SetVolatile_Channel(130);
        Debug_out("Set volatile Channel to 110", ret);
        delay(500);

        ret = ThebeII_SetVolatile_DestNetID(1);
        Debug_out("Set volatile DestNetID to 1", ret);
        delay(500);

        ret = ThebeII_SetVolatile_DestAddr(0x00,0x00);
        ...
        uint8_t txpower = 10;
        ret = ThebeII_GetDefaultTXPower(&txpower);
        Debug_out("ThebeII_GetDefaultTXPower", ret);
        fprintf (stdout,"TXPower is %d dBm\n",txpower);
        delay(500);
        ret = ThebeII_SetDefaultTXPower(18);
        Debug_out("ThebeII_SetDefaultTXPower to 18dBm", ret);
        delay(500);

        uint8_t channel = 0;
        ret = ThebeII_GetDefaultRFChannel(&channel);
        Debug_out("ThebeII_GetDefaultRFChannel", ret);
        fprintf (stdout,"RFChannel is %d\n",channel);
        delay(500);
        ret = ThebeII_SetDefaultRFChannel(129);
        Debug_out("ThebeII_SetDefaultRFChannel to 129", ret);
        delay(500);

        uint8_t srcnetid = 0;
        ret = ThebeII_GetSourceNetID(&srcnetid);
        Debug_out("ThebeII_GetSourceNetID", ret);
        fprintf (stdout,"Source net ID is %d\n",srcnetid);
        delay(500);
        ret = ThebeII_SetSourceNetID(17);
        Debug_out("ThebeII_SetSourceNetID to 17", ret);
        delay(500);

        uint8_t destnetid = 0;
        ret = ThebeII_GetDefaultDestNetID(&destnetid);
        Debug_out("ThebeII_GetDefaultDestNetID", ret);
        fprintf (stdout,"Destination net ID is 0x%x\n",destnetid);
        delay(500);
        ret = ThebeII_SetDefaultDestNetID(0x12);
        Debug_out("ThebeII_SetDefaultDestNetID to 0x12", ret);
        delay(500);

        uint8_t srcaddr_lsb = 0, srcaddr_msb = 0;
        ret = ThebeII_GetSourceAddr(&srcaddr_lsb,&srcaddr_msb);
        Debug_out("ThebeII_GetSourceAddr", ret);
        fprintf (stdout,"Source address is 0x%x%x\n",srcaddr_lsb,srcaddr_msb);
        delay(500);
        ret = ThebeII_SetSourceAddr(0x13,0x14);
        Debug_out("ThebeII_SetSourceAddr to 0x1314", ret);
        delay(500);

        uint8_t destaddr_lsb = 0, destaddr_msb = 0;
        ret = ThebeII_GetDefaultDestAddr(&destaddr_lsb,&destaddr_msb);
        Debug_out("ThebeII_GetDefaultDestAddr", ret);
        fprintf (stdout,"Destination address is 0x%x%x\n",destaddr_lsb,destaddr_msb);
        delay(500);
        ret = ThebeII_SetDefaultDestAddr(0x20,0x21);
        Debug_out("ThebeII_SetDefaultDestAddr to 0x2021", ret);
        delay(500);

        uint8_t profile = 0;
        ret = ThebeII_GetDefaultRFProfile(&profile);
        Debug_out("ThebeII_GetDefaultRFProfile", ret);
        fprintf (stdout,"RF profile is %d\n",profile);
        delay(500);
        ret = ThebeII_SetDefaultRFProfile(2);
        Debug_out("ThebeII_SetDefaultRFProfile to 2", ret);
        delay(500);

        ret = ThebeII_PinReset();
        Debug_out("ThebeII_PinReset", ret);
        delay(500);
        ...
        ret = ThebeII_FactoryReset();
        Debug_out("ThebeII_FactoryReset", ret);
        delay(500);
        ...
        ret = ThebeII_Transmit_Extended(payload,sizeof(payload),129,0,0,0);
        Debug_out("Transmit extended", ret);
        delay(500);
    }
    ret = ThebeII_Deinit();
    Debug_out("ThebeII_Deinit", ret);
}
